...
Texture::Texture(const void* data, TextureSpec spec, const std::string& name) : m_Spec(spec), m_Name(name)
{
	...
	createImage2DInternal(spec.m_Width, spec.m_Height, mips, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_TextureHandle.m_TextureImage, m_TextureHandle.m_TextureImageMemory);

	// Transition image to a state for data to get INTO it
	wVkHelpers::transitionImageLayout(m_TextureHandle.m_TextureImage, VK_FORMAT_R8G8B8A8_SRGB, mips, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	...
	if (generateMips)
		wVkHelpers::generateMipmaps(m_TextureHandle.m_TextureImage, VK_FORMAT_R8G8B8A8_SRGB, spec.m_Width, spec.m_Height, mips);
	else
		wVkHelpers::transitionImageLayout(m_TextureHandle.m_TextureImage, VK_FORMAT_R8G8B8A8_SRGB, 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

	m_TextureHandle.m_TextureImageView = wVkHelpers::createImageView(m_TextureHandle.m_TextureImage, mips, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
